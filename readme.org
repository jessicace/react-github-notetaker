...
* Transitions with React Router
  ...
  7. Over in our Main component, replace the static Menu text with our
     ...
     #+BEGIN_SRC js
       ...
       module.exports = Main;     
     #+END_SRC
* Making Server Requests in React with Axios

  1. Install [[https://github.com/mzabriskie/axios][Axios]]. Axios is a promise-based HTTP client that allows
     us to  make network requests. 
     #+BEGIN_SRC sh
     npm install --save axios@0.8.0
     #+END_SRC
     
** Promises
  - Here's a quick overview of [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise][Promises]].
  - Generally, a promise is an [[http://dictionary.reference.com/browse/promise]['indication of what may be expected']].
  - 'I will study more JavaScript in 2016 than I did in 2015' is a
    promise to myself. Once I'm successful, I want to reward myself
    with some cheesecake. At this stage, it is /pending/- I haven't
    failed it yet, as 2016 only just started! It's too early to
    say. If 2016 ends and I haven't studied any JavaScript, we can say
    that my Promise has been /rejected/. Let's say I'm successful-
    once I pass the amount I studied in 2015, the Promise has been
    /fulfilled/. But it's not quite as simple as waiting until 2017
    comes around, the Promise could be fulfilled by January 2016- and 
    I'm not waiting 11 months for some cheesecake if I can have some
    sooner. Once it's /fulfilled/, then I can have some cheesecake!
  - In JavaScript land, you can create a Promise object by passing in
    a function. Use it for asynchronous computations.  

** Helper for Axios
  1. Create a helpers.js file for handling our network requests,
     inside of a new folder called utils. We are concerned with two
     things: getting a user's repos, and getting a
     user's bio. 
     #+BEGIN_SRC js
  ...
  2. Implement the getRepos() function using the [[https://developer.github.com/v3/repos/#list-user-repositories][GitHub API]], passing
     in a username. Using axios, perform a get() request to the GitHub
     API + /users/:username/repos. This function returns a [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise][Promise]] object. 
     #+BEGIN_SRC js
     // app/utils/helpers.js
  ...
  3. Implement the getUserInfo() function using the [[https://developer.github.com/v3/users/#get-a-single-user][GitHub API]], again
     passing in a username. Using axios, we perform a get() request to
     the GitHub API + /users/:username. Again, this function returns a
     [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise][Promise]] object.
     #+BEGIN_SRC js
     // app/utils/helpers.js
  ...
  4. Create and export a helpers object, which contains the
     getGitHubInfo function. This function uses the two Promises
     returned by our helper functions. We use [[https://github.com/mzabriskie/axios#axiosalliterable][axios.all]], which takes
     in an array of Promises and returns a single promise with an
     array of values (corresponding to the results of the original
     functions' computations). We can access this data by passing a
     function to the .then() property of our Promise, a property found
     in every Promise. then() is invoked when the promise gets
     resolved. So once our Promise object is resolved (i.e. after the
     original Promises are resolved), we can return an object
     containing the repos and profile data! Phew.
  ...
  5. Use the getGitHubInfo helper in our Profile component. When this
     component mounts, after it sets up Firebase, we invoke
     helpers.getGitHubInfo and pass in the username from the route
     params. This will return a promise, that we can use .then() on by
     passing in the function we wish to execute after getGitHubInfo is
     resolved. Note that we use .bind(this)- using bind allows us to
     specify what 'this' refers to within the scope of the
     function. When our data is ready, use setState to update our
     state object with the bio and repos data. We can remove the fake
     data now, too.
  ...
  6. As of React 0.14, we can no longer get JSON and show it to the
     view- this is because an Object is not a valid React child. For
     now, update the UserProfile and Repos components to console.log
     the results instead of rendering them.

* Rendering a UI of Dynamic Data
  1. Create an array of list items for each user repository so that we
     can render them. Use Array.prototype.map again- the first
     parameter is the repo itself, and the second is the
     index. Remember that whenever we map over something, React
     requires a key. It's how React optimises when you remove or move
  ...
  2. Conditionally render the properties of each repo in the list
     item. If the repo has an html_url property, we can render a heading with a link to the 
     repository. Because not every repository has a URL associated
     with it, and React JSX does not support if statements, we use the
     && operator for this conditional logic. 
          #+BEGIN_SRC js
       // app/components/GitHub/Repos.js
  ...
  3. Update render's return statement to use our repos list array.
     #+BEGIN_SRC js
       // app/components/GitHub/Repos.js
  ...
  4. We also need to update our UserProfile component in a similar
     fashion. As not every User bio has all the following properties,
     we conditionally render each one. 
     #+BEGIN_SRC js
       // app/components/GitHub/UserProfile.js
